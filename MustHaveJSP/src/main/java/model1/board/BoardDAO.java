...
//검색 조건에 맞는 게시물의 개수를 반환합니다.
	public int selectCount(Map<String, Object> map) {
		...
		try {
			...
			rs.next();
			totalCount = rs.getInt(1); // 첫 벙째 컬럼 갖옴
		} catch (Exception e) {
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		try {
			...
			while (rs.next()) {
				// 한 행(게시물 하나)의 데이터를 DTO 에 저장
				BoardDTO dto = new BoardDTO();
				dto.setNum(rs.getString("num"));

			}
		} catch (Exception e) {
			System.out.println("게시판 조희 중 예외 발생");
			e.printStackTrace();
		}
		return bbs;
	}
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			...
			System.out.println("게시물 입력 중 예외 발행");
			e.printStackTrace();
		}
		return result;
	}
	...
	public void updateVisitCount(String num) {
		...
		try {
			...
			System.out.println("게시물 조회수 증가 중 예외 발행");
			e.printStackTrace();
		}
	}
